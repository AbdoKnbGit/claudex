...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/claudex/workspace/.claudex-profile.json', 30),
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\claudex\\workspace\\.claudex-profile.json', 30),
    '...\\.claudex-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\claudex\\workspace\\.claudex-profile.json',
  });
