...
export async function mcpListHandler(): Promise<void> {
  ...
  if (Object.keys(configs).length === 0) {
    ...
    console.log('No MCP servers configured. Use `tau mcp add` to add a server.');
  } else {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
...
export async function mcpAddFromDesktopHandler(options: {
  ...
  try {
    ...
    if (Object.keys(servers).length === 0) {
      cliOk('No MCP servers found in Tau Desktop configuration or configuration file does not exist.');
    }
    const {
      unmount
